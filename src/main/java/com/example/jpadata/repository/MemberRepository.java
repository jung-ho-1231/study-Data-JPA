...
public interface MemberRepository extends JpaRepository<Member, Long>, MemberRepositoryCustom, JpaSpecificationExecutor<Member> {
    ...
    @Query("select new com.example.jpadata.dto.MemberDto(m.id, m.username, t.name) from Member m join m.team t")
    ...
    @Query(value = "select new com.example.jpadata.dto.MemberDto(m.id, m.username, t.name)  from Member m left join m.team t",
            countQuery = "select count(m.username) from Member m")
    Page<MemberDto> findByAgeWithDto(int age, Pageable pageable);
    ...
    @Query(value = "select * from member m left join team t on t.team_id = m.team_id",
